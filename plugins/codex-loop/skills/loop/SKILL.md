...
This starts a local app-server runtime, starts `codex-loopd` against it, writes the active runtime marker, and opens `codex --remote` with the `CODEX_LOOP_APP_SERVER`, token file, and runtime DB environment needed by `visible_only` loop tasks.
